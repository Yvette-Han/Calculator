...
const resultBox = document.getElementById("result"); //DOM

// resultBox.innerHTML = "test"; 
...
function operator(val){
    if (lastClick === 'operator') {///  === checks for the same type
    ...
    if (prevOperator == '/' && currentNumber.at(-1) == '0'){
        resultBox.innerHTML = "DIVIDE BY 0 ERROR";
        stackForNumber.splice(0, stackForNumber.length);  // set the two stacks empty
        stackForNumber.splice(0, stackForOperator.length); //
        currentNumber = '';
        return;
    }

    while (stackForOperator.length > 0 && (map.get(val) < map.get(stackForOperator.at(-1)))){ //  >  //map.get(stackForOperator.at(-1)) same as peek
